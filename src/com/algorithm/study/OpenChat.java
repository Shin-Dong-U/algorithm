...
public class OpenChat {
	...
	Map<String, String> nicks = new HashMap<String, String>();//key : id , val : nickname

	public String[] solution(String[] record) {
		int msgLen = record.length;
		Queue<String> actionQueue = new LinkedList<String>();//행위 Enter, Leave, Change
		Queue<String> targetQueue = new LinkedList<String>();//대상 ID
		
		for(String s : record) {
			String[] actionIdNick = s.split(" ");//idx 0 = 행위 1 = 대상  [2 = 닉네임]
			String action = actionIdNick[0];
			String target = actionIdNick[1];
			
			if("Enter".equals(action)) {
				nicks.put(target, actionIdNick[2]); 

				actionQueue.offer(action);
				targetQueue.offer(target);
			}else if("Change".equals(action)) { 
				nicks.put(target, actionIdNick[2]);
				msgLen--; //닉 변경 : 반영은 하되 로그에 추가 메시지를 남기지 않는다. 
		...
		String[] answer = new String[msgLen];
		...
		for(int i = 0; i < msgLen; i++) {
			...
			switch(action) {
			case "Enter" :
				msg = nicks.get(target) + "님이 들어왔습니다.";
				break;
			case "Leave" :
				msg = nicks.get(target) + "님이 나갔습니다.";
				break;
			}
	...
	public static void main(String[] args) {
		...
		String[] answer = oc.solution(record);
